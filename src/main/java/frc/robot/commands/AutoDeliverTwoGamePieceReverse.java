...
public class AutoDeliverTwoGamePieceReverse extends SequentialCommandGroup {
 
  ElevatorSubsystem elevator;
  IntakeSubsystem intake;
  DrivetrainSubsystem drive;
  ArmSubsystem arm;
  WristSubsystem wrist; /** Creates a new AutoDeliverTwoGamePieceReverse. */
  public AutoDeliverTwoGamePieceReverse(ElevatorSubsystem elevator, IntakeSubsystem intake, DrivetrainSubsystem drive, ArmSubsystem arm, WristSubsystem wrist) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
      new AutoDeliverConeTopCommand(elevator, arm, wrist, intake),
      //new AutoMoveCommand(-174, drive, .6, .25),
      new AutoMoveStraightCommand(drive, new PIDController(0.0005, 0.0, 0.0), new PIDController(0.05, 0.0, 0.0), -162),
      new AutoTurnCommand(drive, 142, 0.05),
     ...
     // new SetArmExtensionCommand(ArmConstants.MAX_EXTENSION, arm),
      ...
      new AutoMoveStraightCommand(drive, new PIDController(0.0005, 0.0, 0.0), new PIDController(0.05, 0.0, 0.0), 161),
